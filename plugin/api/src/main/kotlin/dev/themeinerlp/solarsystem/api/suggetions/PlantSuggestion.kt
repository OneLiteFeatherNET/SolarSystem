package dev.themeinerlp.solarsystem.api.suggetions
...
class PlantSuggestion {
    ...
    fun platenSuggestions(asteroid: CommandContext<Asteroid<World>>, input: String): List<String> = transaction {
        return@transaction StringUtil.copyPartialMatches(input, PlanetEntity.all().filter { Bukkit.getWorld(it.name) != null }.map { it.name }, mutableListOf())
    }
}